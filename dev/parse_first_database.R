...
forget_FANTOM5 = function(){
  ...
  onto = ontologyIndex::get_ontology(
    "dev/database/FANTOM5/ff-phase2-140729.obo.txt",
    propagate_relationships = "is_a",
    extract_tags = "minimal"
  )
  
  reads = read_delim(
    "dev/database/FANTOM5/hg19.cage_peak_phase1and2combined_counts_ann.osc.txt.uncommented",
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ) %>%
  ...
  reads %>%
    # Attach ontology
    left_join(
      read_csv("~dev/database/FANTOM5/FANTOM5_annotation_cell_types.csv")
    ) %>%
...
get_bloodRNA = function(){
  ...
  load("dev/database/blood_RNAseq//humanRNASeqCnts_CdG160630.rda")
...
get_immune_singapoor = function(){
  ...
  load("dev/database/immune_singapoor/Genes_TPM_counts.RData")
  ...
  kallisto_gene %$% counts %>%
    ...
    left_join(
      kallisto_gene %$% samples %>% as_tibble %>%
        ...
        left_join(
          read_csv("dev/database/immune_singapoor/cell_type_conversion.csv")
        )
    ) %>%
...
get_dendritic_STIMULATED_NOT_GOOD = function(){
  read_csv("dev/database/mDC_database/GSE89442_Mathan_et_al_RNAseq_data_raw_values.csv") %>%
...
get_immune_skin = function(){
  ...
  read_csv("dev/database/immune_skin/counts_paired_end.csv") %>%
...
get_NK_curated_yuhan = function(){
  ...
  read_csv("dev/database/NK_curated_yuhan/Ste_Alex_GEO_RNA_seq.csv") %>%
    ...
    # Add priming state
    left_join( read_csv("dev/database/NK_curated_yuhan/priming_type.csv")) %>%
...
get_mast_cell = function(){
  ...
  dir(
    path = "dev/database/mast_cell_GSE125887/",
    pattern="tsv",
    full.names = T
  ) %>%
    map_dfr(
...
get_melanocytes = function(){
  ...
  dir(
    path = "dev/database/melanocyte_GSE71747_cultured",
    pattern="txt$",
    full.names = T
  ) %>%
    map_dfr(
...
get_monocytes_brain = function(){
  ...
  GSE88888 <-
    read_delim("dev/database/immune_monocyte_2/raw.txt",  "\t", escape_double = FALSE, trim_ws = TRUE) %>%
...
counts_first_db_raw = 
  ...
  filter(!(
    (cell_type == "dendritic_myeloid" & database == "Immune Singapoor") |
