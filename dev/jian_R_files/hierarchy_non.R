...
saveRDS(contrast_NH_MC, "contrast_NH_mean_contrast.rds")

contrast_NH_MC <- readRDS("intermediate_data/contrast_NH_mean_contrast.rds")
...
sil_tb0_mc <-
  tibble(sig_size = 1:sig_size) %>%
  # slice(1) %>%
  mutate(sil_df = map(sig_size, ~ sig_select0(contrast_MC_NH, .x))) %>%
  mutate(sil_df = map(sil_df, ~ sil_func0(.x, "PCA")))

mc_naive_sil_NH <- sil_tb0_mc %>% 
  unnest(sil_df) %>%
...
saveRDS(mc_naive_sil_NH, "mc_naive_sil_NH.rds")
...
saveRDS(contrast_NH_PW, "contrast_NH_pairwise.rds")
...
sil_tb0_pw <-
  tibble(sig_size = 1:sig_size) %>%
  # slice(1) %>%
  mutate(sil_df = map(sig_size, ~ sig_select0(contrast_PW_NH, .x))) %>%
  mutate(sil_df = map(sil_df, ~.x %>% sil_func0("PCA")))

pw_naive_sil_NH <- sil_tb0_pw %>% 
  unnest(sil_df) %>%
  mutate(ancestor = level_0) %>% 
  mutate(cumulative_signature = map(
    rdim,
...
saveRDS(pw_naive_sil_NH, "pw_naive_sil_NH.rds")

sil_data0 <- sil_tb0_pw %>%
  unnest(sil_df) %>%
...
pw_markers_NH <- contrast_PW_NH %>% 
  single_marker_pw_select0(.discard_num = 1000, METHOD)

mc_markers_NH <- contrast_MC_NH %>% 
  single_marker_pw_select0(.discard_num = 1000, METHOD)

saveRDS(mc_markers_NH, "mc_markers_NH.rds")
...
mc_markers_NH %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("Non-hierarchical, 206 marker selected")
...
mc_markers_NH %>% 
  ...
  mutate(silhouette_data = map2(
    ...
    ~ silhouette_for_markers0(.x, .y, contrast_MC_NH, METHOD)
  )) %>% 
  ...
  # stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA Non-hierarchical MC, 206 markers selected, silhouette =0.176")
