...
# import data
contrast_MC_H <- readRDS("intermediate_data/contrast_MC_H.rds")
contrast_PW_H <- readRDS("intermediate_data/contrast_PW_H.rds")
contrast_MC_NH <- readRDS("intermediate_data/contrast_MC_NH.rds")
contrast_PW_NH <- readRDS("intermediate_data/contrast_PW_NH.rds")

METHOD <- "PCA"

# execution

mean_contrast.naive.non_hierarchy.unOP.new <- contrast_MC_NH %>% 
...
pairwise.naive.non_hierarchy.unOP.new <- contrast_PW_NH %>% 
  do_ranking(.ranking_method = "logFC") %>%
  do_selection("naive", .kmax = 60, METHOD) %>% 
  # do_optimisation("penalised") %>% 
  format_output(.is_complete = TRUE)
...
# ====================================
mean_contrast.naive.hierarchy.unOP.new <- contrast_MC_H %>% 
  do_ranking(.ranking_method = "logFC") %>%
  do_selection("naive", .kmax = 60, METHOD) %>% 
  # do_optimisation("penalised") %>% 
  format_output(.is_complete = TRUE)
...
pairwise.naive.hierarchy.unOP.new <- contrast_PW_H %>% 
  do_ranking(.ranking_method = "logFC") %>%
  do_selection("naive", .kmax = 60, METHOD) %>% 
  # do_optimisation("penalised") %>% 
  format_output(.is_complete = TRUE)
...
# ======================================
mean_contrast.silhouette.non_hierarchy.unOP.new <- contrast_MC_NH %>% 
  do_ranking(.ranking_method = "logFC") %>%
