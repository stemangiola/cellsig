...
saveRDS(pw_naive_sil_L5, "pw_naive_sil_L5.rds")
...
saveRDS(mc_naive_sil_L5, "mc_naive_sil_L5.rds")
...
pw_markers_L5 <- contrast_PW_L5 %>%
  single_marker_pw_select(LEVEL, .discard_num = 1000, METHOD)

saveRDS(pw_markers_L5, "pw_markers_L5.rds")

mc_markers_L5 <- contrast_MC_L5 %>%
  single_marker_pw_select(LEVEL, .discard_num = 1000, METHOD)

saveRDS(mc_markers_L5, "mc_markers_L5.rds")

# pw_markers_L5 trend plots & PCA ==================

# trend plot t_CD4_memory
pw_markers_L5 %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("t_CD4_memory, 50 marker selected")

# PCA plot t_CD4_memory
pw_markers_L5 %>% 
  slice(1) %>% 
  ...
  mutate(silhouette_data = map2(
    ...
    ~ silhouette_for_markers(.x, .y, contrast_PW_L5, LEVEL, METHOD)
  )) %>% 
  ...
  stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA t_CD4_memory, 50 markers selected, silhouette = 0.934")

# trend plot t_CD8_memory
pw_markers_L5 %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("t_CD8_memory, 16 marker selected")

# PCA plot t_CD8_memory
pw_markers_L5 %>% 
  slice(2) %>% 
  unnest(signature_data) %>% 
  tail(1) %>%
  ...
  stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA t_CD8_memory, 16 markers selected, silhouette = 0.687")
...
pw_markers_L5 %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("t_helper, 72 marker selected")
...
pw_markers_L5 %>% 
  ...
  stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA t_helper, 72 markers selected, silhouette = 0.832")

# pw_markers_L5.2 (base markers = 2) =================
# data output from new method

pw_markers_L5.2 <- contrast_PW_L5 %>%
  single_marker_pw_select(LEVEL, .discard_num = 1000, METHOD)

saveRDS(pw_markers_L5.2, "pw_markers_L5.2.rds")

# pw_markers_L5.2 trend plots & PCA
...
pw_markers_L5.2 %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("t_CD8_memory, 25 marker selected")
...
pw_markers_L5.2 %>% 
  ...
  stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA t_CD8_memory, 25 markers selected, silhouette = 0.658")
...
pw_markers_L5.2 %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("t_helper, 188 marker selected")
...
pw_markers_L5.2 %>% 
  ...
  stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA t_h,elper 188 markers selected, silhouette = 0.855")




# mc_markers_L5 (base marker = 1) trend plot & PCA===============
...
mc_markers_L5 %>% 
  ...
  mutate(silhouette_data = map2(
    cumulative_signature, ancestor,
    ~ silhouette_for_markers(.x, .y, contrast_MC_L5, LEVEL, METHOD)
  )) %>% 
  pluck("silhouette_data", 1) %>% 
  pluck("reduced_dimensions", 1) %>% 
  ggplot(aes(x = PC1, y = PC2, colour = !!as.symbol(LEVEL), label = sample)) + 
...
x <- mc_markers_L5 %>% 
  ...
  geom_point() +
  theme_bw() +
  ggtitle("t_helper, 92 marker selected")
ggsave("x.png", x)
...
x <- mc_markers_L5 %>% 
  ...
  stat_ellipse(type = 't')+
  theme_bw() +
  ggtitle("PCA t_helper, 92 markers selected, silhouette = 0.843")
