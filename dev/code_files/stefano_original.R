...
tt <- 
  ...
  ## the tibble is 5 x 2: 5 levels and two columns: 'level' and data', 'data' is now a list of tibbles
  nest(data = -level) %>% 

# Redefine factors inside each level
## why do we need to droplevels on all tibbles of the data list?
...
all_contrasts %>%
  ...
  tidybulk::reduce_dimensions(sample, symbol, count_scaled,  method = "PCA", action="get", transform = log1p, scale=FALSE) %>%
  ggplot(aes(x = PC1, y = PC2, colour = cell_type)) + 
  geom_point() +
  # expand_limits(x=c(-5, 10), y=c(-10, 15)) +
  theme_bw()
