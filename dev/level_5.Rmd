---
title: "Analysis Summary level_5"
author: "Jian"
date: "26/01/2021"
...
## 2.3 Scaled PC values for silhouette analysis

```{r sil_func2, message=F}
sil_func2 <- function(.sil_df, LEVEL){
  .sil_df %>% 
    nest(markers = - !!as.symbol(pre(LEVEL))) %>% 
    mutate(pca = map(markers, ~ .x %>% 
                       distinct(sample, symbol, count_scaled, !!as.symbol(LEVEL)))) %>% 
    ...
    # calculate the dissimilarity matrix with PC values
    mutate(distance = map(pca_norm, ~ .x %>% 
                            select(contains("PC")) %>% 
                            dist()
    )) %>% 
    
    # calculate silouette score
    mutate(sil = map2(pca_norm, distance, 
                      ~ silhouette(as.numeric(as.factor(`$`(.x, !!as.symbol(LEVEL)))), .y)
    )) %>% 
    mutate(sil_info = map(sil, ~ .x %>% summary())) %>% 
    mutate(sil_score = map(sil_info, ~ .x %>% `$`(avg.width))) %>% 
    mutate(sil_score = unlist(sil_score)) %>% 
    mutate(real_size=map_int(pca_norm, ~ n_distinct(.x$symbol) ))
  
}

```

```{r silhouette_2, results = FALSE, warning = FALSE, message=FALSE}
sig_size <- 20
...
### 2.3.1 Signature size selection & PCA plot for t_CD4_memory cell

Signature size selection plot for t_CD4_memory cell:

```{r tCD4_memory_sil2, message=F}
tCD4_memory_sil2 <- sil_data %>%
  pluck("plot_data", 1) %>%
  ggplot(aes(real_size, sil_score)) +
  geom_line() +
  geom_point()
...
The peak is reached when sig_size = $14$. PCA at sig_size = $14$:

```{r PCA5_tCD4_memory_sil2, warning = FALSE, message=F}
sig_size <- 14
...
PCA5_tCD4_memory_sil2 <- PCA_level5 %>%
  pluck("pca", 1) %>%
  ggplot(aes(x = PC1, y = PC2, colour = !!as.symbol(LEVEL), label = sample)) +
  geom_point() +
  stat_ellipse(type = 't')+
...
PCA5_tCD4_memory_sil2

```

### 2.3.2 Signature size selection & PCA plot for t_CD8_memory cell

Signature size selection plot for t_CD8_memory cell:

```{r tCD8_memory_sil2, message=F}
tCD8_memory_sil2 <- sil_data %>%
  pluck("plot_data", 2) %>%
  ggplot(aes(real_size, sil_score)) +
  geom_line() +
  geom_point()
...
```{r PCA5_tCD8_memory_sil2, warning = FALSE, message=F}
sig_size <- 5
...
PCA5_tCD8_memory_sil2 <- PCA_level5 %>%
  pluck("pca", 2) %>%
  ggplot(aes(x = PC1, y = PC2, colour = !!as.symbol(LEVEL), label = sample)) +
  geom_point() +
  stat_ellipse(type = 't')+
...
PCA5_tCD8_memory_sil2
```

### 2.3.3 Signature size selection & PCA plot for t_helper cell

Signature size selection plot for t_helper cell:

```{r t_helper_sil2, warning = FALSE, message=F}
t_helper_sil2 <- sil_data %>%
  pluck("plot_data", 3) %>%
  ggplot(aes(real_size, sil_score)) +
  geom_line() +
  geom_point()
...
```{r PCA5_t_helper_sil2, warning = FALSE, message=F}
sig_size <- 3
