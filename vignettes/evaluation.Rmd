...
create_random_mix_signature_input <- function(.x, .sample, .cell_type, .database,
  ...
  foreach(i = 1:.iteration) %do% {
    ...
    saveRDS(sig_frac, paste0("sig_input_round_", .n, .suffix, ".rds"), compress = "xz")
    ...
    saveRDS(mix, paste0("mix_input_round_", .n, .suffix, ".rds"), compress = "xz")
...
```{r}
create_perturbed_signature_for_evaluation <- function(input_directory, .frac){
