...
get_markers_from_transcription_abundance_quantiles <- function(.input, .sample, .symbol, .count=NULL, .cell_type,
                                                               .lower_quantile, 
                                                               .upper_quantile, 
                                                               .tree,
                                                               .is_hierarchy = TRUE,
  ...
  bayes_quantile = .input %>%
    select(colnames(bayes)) %>% distinct() %>%
    dplyr::rename(sample = sample_bayes, lower_quantile = as.name(.lower_quantile), upper_quantile = as.name(.upper_quantile))
  

  .input %>% 
    
    select(sample, count, cell_type, symbol) %>% drop_na() %>%
    
    main(.sample = !!.sample, .symbol = !!.symbol, .count = !!.count, .cell_type = !!.cell_type,
       .is_hierarchy = FALSE, 
       .tree = .tree,
       .contrast_method = .contrast_method, .ranking_method = rank_bayes, .bayes= bayes_quantile,
...
bayes = readRDS(here("dev/jian_R_files/test_count_bayes.rds"))  %>%
  ...
  unite(sample_bayes, id, sample, sep = "_")
...
marker_genes = 
  ...
  count %>%
  full_join(bayes, by = c("cell_type", "symbol")) %>%
  
  get_markers_from_transcription_abundance_quantiles(
    .sample = sample, .symbol = symbol, .count= count,
    .cell_type = cell_type,
    .lower_quantile = "10%",
    .upper_quantile = "90%",
    .tree = tree,
